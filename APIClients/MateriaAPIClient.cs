...
namespace APIClients
{
    public class MateriaAPIClient
    {
        ...
        public static async Task<bool> ExistPlanAndDescripcionMateriaAsync(int idPlan, string descripcionMateria, int? excludeId = null)
        {
            try
            {
                ...
                {
                    string errorContent = await response.Content.ReadAsStringAsync();
                    throw new Exception($"Error al verificar plan y descripción de materia duplicados. Status: {response.StatusCode}, Detalle: {errorContent}");
                }
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Error de conexión al verificar plan y descripción de materia duplicados: {ex.Message}", ex);
            }
            catch (TaskCanceledException ex)
            {
                throw new Exception($"Timeout al verificar plan y descripción de materia duplicados: {ex.Message}", ex);
            }
        }
    }
}
